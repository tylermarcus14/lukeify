...
export class LukeifyService {
    ...
    /**
     * Send a post request which gets in touch with me, by sending an email to somewhere secret.
     *
     * @returns {Observable<boolean>} - Whether the email was successfully sent or not.
     */
    public getInTouch(formData: any) : Observable<boolean> {
        return this.http.post('/api/getintouch', formData, {observe: "response"}).pipe(
